...
local EXCLAIM, QUOT, APOS, MINUS, SLASH, LT, EQ, GT, QUESTION, LSQUARE, RSQUARE =
    sbyte("!\"'-/<=>?[]", 1, 11)
...
local function isname(c)
    ...
    return c == 45 or c == 46 or c == 58 or c == 95 or (c >= 48 and c <= 57) or
...
local function parse_xml(s, preserve_white)
    ...
    local function emit_open_tag(s) tinsert(stack, {tag = s, attr = {}}) end

    local function emit_att(k, v) stack[#stack].attr[k] = unescape(v) end
    ...
    do
        ...
        mark = p
        while p <= n do
            if sbyte(s, p) == MINUS and sbyte(s, p + 1) == MINUS and
                sbyte(s, p + 2) == GT then
                p = p + 3
                goto parse_text
            end
    ...
    do
        ...
        mark = p
        while p <= n do
            if sbyte(s, p) == RSQUARE and sbyte(s, p + 1) == RSQUARE and
                sbyte(s, p + 2) == GT then
                if p > mark then emit_text(ssub(s, mark, p - 1)) end
                p = p + 3
                goto parse_text
