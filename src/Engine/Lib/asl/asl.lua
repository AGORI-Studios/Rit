...
-- Safeguards.
do
    local M, m, r = love_getVersion()
    assert((M == 11 and m >= 3) or M >= 12, "This library needs at least LÖVE 11.3 to function.")
    assert(love.audio and love.sound, "This library needs both love.audio and love.sound enabled to function.")
end
...
local transfer = function(instance, ...)
    ...
    table_remove(instance.callStack, 1)
    ...
    for i = 1, retvalCount do
        table_insert(retval, toHere:pop())
    end
...
mt = {
    __index = function(instance, m)
        if method[m] then
            ...
            table_insert(instance.callStack, 1, m)
            return transfer
        end
    end
}
...
function love.audio.play(...)
    local temp = select(1, ...)
    if type(temp) == "table" and temp.typeOf then
        temp = {...}
    end
...
function love.audio.pause(...)
    if select("#", ...) == 0 then
        ...
        for i = #temp2, 1, -1 do
            if temp2[i] == false then
                table_remove(temp2, i)
            end
        end
