local require, type, error, tostring, setmetatable, pairs, ipairs, pcall, table_insert, loadstring, unpack =
    require,
    type,
    error,
    tostring,
...
local MixMethodIMap = {}
for i = 0, #MixMethodList do
    MixMethodIMap[MixMethodList[i]] = i
end

-- Buffer variance distribution method list and reverse-lookup table.
local VarDistType = {[0] = "uniform", "normal"}
local VarDistIMap = {}
for i = 0, #VarDistType do
    VarDistIMap[VarDistType[i]] = i
end
...
local function calculateTSMCoefficients(instance)
    -- Set offsets
    instance.innerOffset = instance.resampleRatio * instance.pitchShift * math.sgn(instance.timeStretch)
    instance.outerOffset =
        instance.resampleRatio *
        (instance.timeStretch * math.sgn(instance.resampleRatio) - instance.pitchShift * math.sgn(instance.timeStretch))
...
Process.static = function(instance)
    ...
    for b = 0, instance.bufferSize - 1 do
        ...
        if not instance.looping then
            ...
            if not instance.loopRegionEntered then
                ...
                if not disjunct then
                    ...
                    if
                        (smpOffset >= 0 and smpOffset <= instance.loopRegionB) or
                            (smpOffset <= N - 1 and smpOffset >= instance.loopRegionA)
                     then
                        instance.loopRegionEntered = true
                    end
                end
            ...
            if instance.loopRegionEntered then
                ...
                smpOffset =
                    (((smpOffset - instance.loopRegionA) % N + i * instance.innerOffset) % loopRegionSize +
                    instance.loopRegionA) %
                    N

                mixSmpOffset =
                    (((smpOffset - instance.loopRegionA) % N + mixFrameOffset) % loopRegionSize + instance.loopRegionA) %
                    N
            else
                smpOffset = frameOffset + i * instance.innerOffset
                mixSmpOffset = smpOffset + mixFrameOffset
        ...
        if instance.channelCount == 1 then
            ...
            elseif itplMethodIdx == 1 then
                ...
                frac = smpOffset - int
                A = instance.data:getSample(int % N) * (1.0 - frac) + instance.data:getSample((int + 1) % N) * frac

                int = math.floor(mixSmpOffset)
                frac = mixSmpOffset - int
                B = instance.data:getSample(int % N) * (1.0 - frac) + instance.data:getSample((int + 1) % N) * frac
            elseif itplMethodIdx == 2 then
                ...
                for t = -taps + 1, taps do
                    ...
                    result = result + instance.data:getSample(i) * lanczos_window(offset - i, taps)
                end
                A = result
            ...
            elseif itplMethodIdx == 1 then
                ...
                frac = smpOffset - int
                AL =
                    instance.data:getSample(int % N, 1) * (1.0 - frac) +
                    instance.data:getSample((int + 1) % N, 1) * frac
                AR =
                    instance.data:getSample(int % N, 2) * (1.0 - frac) +
            ...
            elseif itplMethodIdx == 2 then
                ...
                for t = -taps + 1, taps do
                    local i = math.floor(smpOffset + t) % N
                    resultL = resultL + instance.data:getSample(i, 1) * lanczos_window(offset - i, taps)
                    resultR = resultR + instance.data:getSample(i, 2) * lanczos_window(offset - i, taps)
                end
        ...
        if instance._frameIndex > instance.curFrameSize - 1 then
            ...
            local nextPlaybackOffset = instance.playbackOffset + instance.curFrameSize * instance.frameAdvance
            ...
            if instance.frameVarianceDistributionIdx == 0 then
                ...
                instance.curFrameSize = math.floor(math.clamp(normal, instance.minFrameSize, instance.maxFrameSize))
            end
        end
    end
    ...
    instance.source:queue(instance.buffer, instance.bufferSize * (instance.bitDepth / 8) * instance.outputAurality)
...
local function new(a, b, c, d, e)
    ...
    if type(a) == "nil" then
        error(
            "ASource constructor: Missing 1st parameter; it can be one of the following:\n" ..
                "string, File, FileData, Decoder, SoundData; use number for a queue-type instance."
        )
    elseif type(a) == "number" then
        -- Queueable type_
        if type(b) ~= "number" then
            error(("ASource constructor: 2nd parameter must be a number; " .. "got %s instead."):format(type(b)))
        end
        if type(c) ~= "number" then
            error(("ASource constructor: 3rd parameter must be a number; " .. "got %s instead."):format(type(c)))
        end
        if type(d) ~= "number" then
            error(("ASource constructor: 4th parameter must be a number; " .. "got %s instead."):format(type(d)))
        end
        if not ({[8] = true, [16] = true})[b] then
            error(("ASource constructor: 2nd parameter must be either 8 or 16; " .. "got %f instead."):format(b))
        end
        if not ({[1] = true, [2] = true})[c] then
            error(("ASource constructor: 3rd parameter must be either 1 or 2; " .. "got %f instead."):format(c))
        end
        sourcetype = "queue"
    elseif
        type(a) == "string" or a.type and a:type() == "File" or a.type and a:type() == "DroppedFile" or
     ...
     then
        ...
        if type(b) ~= "string" then
            error(("ASource constructor: 2nd parameter must be a string; " .. "got %s instead."):format(type(b)))
        end
        if not ({static = true, stream = true})[b] then
            error(
                ("ASource constructor: 2nd parameter must be either `static` or `stream`; " .. "got %s instead."):format(
                    tostring(b)
                )
            )
        end
    ...
    elseif a.type and a:type() == "SoundData" then
        -- Can only be static type_
        sourcetype = "static"
    else
        error(
            ("ASource constructor: 1st parameter was %s; must be one of the following:\n" ..
    ...
    if sourcetype == "queue" then
        ...
        if
         ...
         then
            ...
            elseif type(c) ~= "nil" then
                error(
                    ("3rd, optional parameter expected to be a number between 1 and 64; " .. "got %s instead."):format(
                        type(c)
                    )
                )
            end
        elseif a.type and a:type() == "SoundData" then
            ...
            elseif type(b) ~= "nil" then
                error(
                    ("2nd, optional parameter expected to be a number between 1 and 64; " .. "got %s instead."):format(
                        type(b)
                    )
                )
            end
    ...
    if sourcetype == "queue" then
        ...
        if
         ...
         then
            ...
            elseif type(d) ~= "nil" then
                error(
                    ("4th, optional parameter expected to be a number, either 1 or 2; " .. "got %s instead."):format(
                        type(d)
                    )
                )
            end
        elseif a.type and a:type() == "SoundData" then
            ...
            elseif type(c) ~= "nil" then
                error(
                    ("3rd, optional parameter expected to be a number, either 1 or 2; " .. "got %s instead."):format(
                        type(c)
                    )
                )
            end
    ...
    instance.buffer =
        love.sound.newSoundData(instance.bufferSize, instance.samplingRate, instance.bitDepth, instance.outputAurality)
...
function ASource.clone(instance)
    ...
    clone.buffer = love.sound.newSoundData(clone.bufferSize, clone.samplingRate, clone.bitDepth, clone.channelCount)
    ...
    clone.source =
        love.audio.newQueueableSource(clone.samplingRate, clone.bitDepth, clone.outputAurality, clone.OALBufferCount)
...
function ASource.setPitch(instance)
    error(
        "Function deprecated by Advanced Source Library; " ..
...
function ASource.getBufferSize(instance, unit)
    ...
    if not BufferUnit[unit] then
        error(
            ("1st parameter must be `milliseconds`, `samples` or left empty; " .. "got %s instead."):format(
                tostring(unit)
            )
        )
    end
...
function ASource.setBufferSize(instance, size, unit)
    ...
    if not BufferUnit[unit] then
        error(
            ("2nd parameter must be `milliseconds`, `samples` or left empty; " .. "got %s instead."):format(
                tostring(unit)
            )
        )
    end
    ...
    if type(size) ~= "number" then
        error(("1st parameter must be a non-negative number; " .. "got %s instead."):format(tostring(size)))
    end
    ...
    if size < min or size > max then
        error(("1st parameter out of range; " .. "min/given/max: %f < [%f] < %f (%s)."):format(min, size, max, unit))
    end
...
function ASource.getFrameSize(instance, unit)
    ...
    if unit == "samples" then
        return instance.frameSize, instance.curFrameSize
    else --if unit == 'milliseconds' then
        return instance.frameSize / instance.samplingRate * 1000, instance.curFrameSize / instance.samplingRate * 1000
    end
end
...
function ASource.getFrameVariance(instance, unit)
    ...
    if not VarianceUnit[unit] then
        error(
            ("1st parameter must be `milliseconds`, `samples`, `percentage` or left empty; " .. "got %s instead."):format(
                tostring(unit)
            )
        )
    end
...
function ASource.setFrameVariance(instance, variance, unit)
    ...
    if not VarianceUnit[unit] then
        error(
            ("2nd parameter must be `milliseconds`, `samples`, `percentage` or left empty; " .. "got %s instead."):format(
                tostring(unit)
            )
        )
    end
    ...
    if type(variance) ~= "number" then
        error(("1st parameter must be a non-negative number; " .. "got %s instead."):format(tostring(variance)))
    end
    if variance < 0 then
        error(("1st parameter must be a non-negative number; " .. "got %f instead."):format(variance))
    end

    if unit == "percentage" and variance > 1.0 then
        error(
            ("1st parameter as a percentage can't be more than 100%; " .. "got %f%% (%f) instead."):format(
                math.floor(variance * 100),
                variance
            )
        )
    end
...
function ASource.setFrameVarianceDistribution(instance, distribution)
    if not VarDistIMap[distribution] then
        error(
            ("1st parameter is not a supported buffer variance distribution; got %s.\n" ..
...
function ASource.tell(instance, unit)
    ...
    if not TimeUnit[unit] then
        error(
            ("1st parameter must be `seconds`, `samples` or left empty; " .. "got %s instead."):format(tostring(unit))
        )
    end
...
function ASource.seek(instance, position, unit)
    ...
    if not TimeUnit[unit] then
        error(
            ("2nd parameter must be `seconds`, `samples` or left empty; " .. "got %s instead."):format(tostring(unit))
        )
    end
    ...
    if type(position) ~= "number" then
        error(("1st parameter must be a non-negative number; " .. "got %s instead."):format(tostring(position)))
    end
    if position < 0 then
        error(("1st parameter must be a non-negative number; " .. "got %f instead."):format(position))
    end
    ...
    if instance.type == "queue" then
        ...
        if unit == "samples" then
            ...
            if position >= limit then
                error(("1st parameter outside of data range; " .. "%f > %f."):format(position, limit))
            end
...
function ASource.rewind(instance)
    if instance.type == "queue" then
        ...
        if instance.timeStretch >= 0 then
            ...
            if instance.type == "static" then
                instance:seek(math.max(0, instance.data:getSampleCount() - 1), "samples")
            else --if instance.type == 'stream' then
                instance:seek(math.max(0, instance.data:getDuration() * instance.samplingRate - 1), "samples")
            end
        end
    end
    return true
...
function ASource.setLooping(instance, state)
    ...
    if type(state) ~= "boolean" then
        error(("1st parameter must be boolean; " .. "got %s instead."):format(type(state)))
    end
...
function ASource.setLoopPoints(instance, pointA, pointB)
    ...
    if pointA ~= nil then
        if (type(pointA) ~= "number" or pointA < 0) then
            error(
                ("1st parameter must be a non-negative number; " .. "got %s of type %s instead."):format(
                    pointA,
                    type(pointA)
                )
            )
        ...
        if pointA >= limit then
            error(("1st parameter must be less than the length of the data; " .. "%f > %f."):format(pointA, limit))
        end
    ...
    if pointB ~= nil then
        if (type(pointB) ~= "number" or pointB < 0) then
            error(
                ("2nd parameter must be a non-negative number; " .. "got %s of type %s instead."):format(
                    pointB,
                    type(pointB)
                )
            )
        ...
        if pointB >= limit then
            error(("2nd parameter must be less than the length of the data; " .. "%f > %f."):format(pointB, limit))
        end
...
function ASource.setInterpolationMethod(instance, method)
    if not ItplMethodIMap[method] then
        error(
            ("1st parameter not a supported interpolation method; got %s.\n" ..
...
function ASource.setMixMethod(instance, method)
    if not MixMethodIMap[method] then
        error(
            ("1st parameter not a supported mixing method; got %s.\n" ..
...
function ASource.setResamplingRatio(instance, ratio)
    ...
    if type(ratio) ~= "number" then
        error(("1st parameter must be a number; " .. "got %s instead."):format(tostring(ratio)))
    end
...
function ASource.getPitchShift(instance, unit)
    ...
    if not PitchUnit[unit] then
        error(
            ("1st parameter must be `ratio`, `semitones` or left empty; " .. "got %s instead."):format(tostring(unit))
        )
    end
...
function ASource.setPitchShift(instance, amount, unit)
    ...
    if not PitchUnit[unit] then
        error(
            ("2nd parameter must be `ratio`, `semitones` or left empty; " .. "got %s instead."):format(tostring(unit))
        )
    end
    ...
    if type(amount) ~= "number" then
        error(("1st parameter must be a number; " .. "got %s instead."):format(tostring(amount)))
    end

    if unit == "ratio" then
        if amount <= 0 then
            error(("1st parameter must be a positive number as a ratio; " .. "got %f instead."):format(amount))
        end
...
function ASource.setPanLaw(instance, law)
    if not law then
        error(
            "Missing 1st parameter, must be `gain`, `power` or a function " ..
    ...
    if type(law) == "string" then
        if not PanLawIMap[law] then
            error(("1st parameter as a string must be `gain` or `power`; " .. "got %s instead."):format(law))
        end
    ...
    elseif type(law) == "function" then
        ...
        for i, v in ipairs {0.00, 0.25, 0.33, 0.50, 0.67, 1.00} do
            ...
            if type(l) ~= "number" or type(r) ~= "number" then
                error(
                    ("The given pan law function must return two numbers; " .. "got %s and %s instead."):format(
                        type(l),
                        type(r)
                    )
                )
            ...
            if l < 0.0 or l > 1.0 or r < 0.0 or r > 1.0 then
                error(
                    ("The given pan law function's return values must be in [0.0,1.0]; " .. "got %f and %f instead."):format(
                        l,
                        r
                    )
                )
        ...
        error(("1st parameter must be a string or a function; " .. "got %s instead."):format(type(law)))
    end
...
function ASource.setPanning(instance, pan)
    ...
    if type(pan) ~= "number" then
        error(("1st parameter must be a number between 0 and 1 inclusive; " .. "got %s instead."):format(tostring(pan)))
    end
    if pan < 0 or pan > 1 then
        error(("1st parameter must be a number between 0 and 1 inclusive; " .. "got %f instead."):format(pan))
    end
...
function ASource.setStereoSeparation(instance, sep)
    ...
    if type(sep) ~= "number" then
        error(
            ("1st parameter must be a number between -1 and 1 inclusive; " .. "got %s instead."):format(tostring(sep))
        )
    end
    if sep < -1.0 or sep > 1.0 then
        error(("1st parameter must be a number between -1 and 1 inclusive; " .. "got %f instead."):format(sep))
    end
    ...
    elseif msg == "pauseall" then
        ...
        for i = 1, #ASourceList do
            if ASourceList[i]:isPlaying() then
                ASourceList[i]:pause()
                table_insert(idList, ASourceList[i].id)
            end
        end
        ch:performAtomic(
            function(ch)
